...
(function () {
  ...
  /* ── iOS hors Safari : dire l'issue au lieu d'une consigne impossible ──
     ...
     mais leur « Ajouter à l'écran d'accueil » ne crée qu'un raccourci : ni
     icône de marque, ni notifications. La liste d'abonnés Progressier le
     montrait sans qu'on l'ait lu — chaque ligne abonnée est « Mobile Safari »,
     la seule ligne « Chrome » n'était pas abonnée.

     Le client qui touche la carte NFC ouvre son navigateur par défaut. Si ce
     n'est pas Safari, la page d'installation lui demandait d'appuyer sur le
     bouton Partager « en bas de Safari » : une instruction qu'il ne pouvait
     pas suivre. Il repartait soit sans carte, soit avec un raccourci mort.
  ...
  (function preveniriOSHorsSafari() {
    ...
    function afficher() {
      ...
      // Chrome, Firefox, Edge, Opera, Brave, DuckDuckGo, Yandex sur iOS.
      var autre = /CriOS|FxiOS|EdgiOS|OPiOS|OPT\/|Brave|DuckDuckGo|YaBrowser/i.test(ua);
      if (!autre) return;

      // Déjà installée : plus rien à expliquer.
      ...
      // Seules les trois pages où un client arrive après le NFC.
      var fichier = location.pathname.split("/").pop() || "index.html";
      if (PAGES.indexOf(fichier) === -1) return;

      if (document.getElementById("fvSafari")) return;

      var nom = /CriOS/i.test(ua)  ? "Chrome"
              : /FxiOS/i.test(ua)  ? "Firefox"
              : /EdgiOS/i.test(ua) ? "Edge"
              : /OPiOS|OPT\//i.test(ua) ? "Opera"
              : "votre navigateur";

      var b = document.createElement("div");
      b.id = "fvSafari";
      b.style.cssText =
        "position:fixed;top:0;left:0;right:0;z-index:99999;padding:14px 16px;" +
        "background:#1B1B1F;color:#fff;font-size:14px;line-height:1.5;" +
        "font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;" +
        "box-shadow:0 2px 12px rgba(0,0,0,.25)";
      b.innerHTML =
        '<div style="max-width:560px;margin:0 auto;position:relative;padding-right:26px">' +
          ...
          '<button id="fvSafariFerme" aria-label="Fermer" style="position:absolute;top:-2px;' +
            'right:-4px;background:none;border:0;color:#fff;font-size:20px;line-height:1;' +
            'opacity:.6;cursor:pointer">×</button>' +
        '</div>';
      document.body.appendChild(b);

      // Le bandeau est fixe : sans ce décalage il masque le haut de la page.
      var hauteur = b.offsetHeight;
      var padOrigine = document.body.style.paddingTop;
      document.body.style.paddingTop = hauteur + "px";

      document.getElementById("fvSafariFerme").onclick = function () {
        b.remove();
        document.body.style.paddingTop = padOrigine;
      };

      document.getElementById("fvSafariCopie").onclick = function () {
        var btn = this;
        function ok() { btn.textContent = "Adresse copiée ✓"; }
        try {
          if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(location.href).then(ok, secours);
          } else { secours(); }
        } catch (e) { secours(); }

        // iOS refuse l'accès au presse-papiers dans certains contextes : on
        // pose alors l'adresse dans un champ sélectionné, prête à être copiée.
        function secours() {
          var z = document.createElement("input");
          z.value = location.href;
          z.style.cssText = "position:fixed;top:-999px";
          document.body.appendChild(z);
          z.select();
          z.setSelectionRange(0, 99999);
          try { document.execCommand("copy"); ok(); }
          catch (e) { btn.textContent = "Copie impossible"; }
          z.remove();
        }
      };
    }
